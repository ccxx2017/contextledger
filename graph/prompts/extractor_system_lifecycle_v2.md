...
> 实验版本：extractor_system_lifecycle_v2（2026-09-06）。
> 冻结版 extractor_system.md 未改动。本版本新增 §二.1 lifecycle 字段，
> 对应冒烟项 S4（lifecycle 进真实抽取链路）。实验结论回填
> contextledger-host/pilot/SMOKE_RESULTS.md。
...
```json
{
  ...
  "lifecycle_ref": "本轮信息所属任务/流程的唯一标识，无则 null",
  "state_slot": "该节点描述实体的哪个可替代状态维度，无则 null",
  "observed_at": "本轮事件时间（ISO 8601，取自本轮时间戳），无则 null",
  "effective_at": "业务上声称的生效时点（ISO 8601），原文未明确给出则 null"
}
```

## 二.1 lifecycle 字段（v2 新增，实验版本）

- `lifecycle_ref`：当多条信息属于同一次任务/流程（同一工单、同一方案流、同一改造批次）时，
  为它们填同一个 lifecycle_ref（如 `lc-plan-a`、`lc-impl-005`）。
  判断标准：**这些信息讲述的是同一件事的推进**。不同任务/方案必须用不同 lifecycle_ref，
  禁止把所有节点塞进同一个 lifecycle_ref。
- `state_slot`：该节点描述实体的哪个**可替代状态维度**。同一流程的不同维度各自独立演进、互不取代：
  - 方案/审批类 → `approval_status`
  - 负责人/归属类 → `owner`
  - 构建/测试进度类 → `build_status`
  - 执行状态类 → `execution_status`
  无明确维度语义时填 null。
- `lifecycle_seq`：**禁止输出**。该字段由机械层分配，LLM 不得生成。
- `observed_at`：取本轮时间戳（Slice 顶部或 raw 标注的【本轮时间】）。
- `effective_at`：仅当原文明确声称"该事实自某时点起生效/早就发生"时填写该时点；
  否则 null。**不要用本轮时间戳冒充 effective_at。**
- 判定口诀：同一 lifecycle_ref + 同一 state_slot = 新值取代旧值；
  同 lifecycle_ref 不同 state_slot = 并存；不同 lifecycle_ref = 互不干扰。
