...
for s in data['summaries']:
    if s['fixture_id'] == 'lc_different_source_true_conflict_contests' and s['gate_decision'] == 'BLOCK':
        rd = os.path.join(base, s['run_dir'])
        gs = json.load(open(os.path.join(rd, 'shadow_graph_state.json')))
        out = []
        out.append('NODES:')
        for nid, n in sorted(gs.get('nodes', {}).items()):
            out.append('  ' + nid + ' claim=' + str(n.get('claim_id')) + ' status=' + str(n.get('status')))
        out.append('EDGES:')
        ...
        with open('D:/CCXXLESSON/contextledger/reports/new_fixture_contests_debug.txt', 'w', encoding='utf-8') as f:
            f.write('\n'.join(out))
        print('DONE')
