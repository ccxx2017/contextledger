...
- full：基线水位为 P/R=`1.000/0.615`，Phase 1 不能回退。
...
- revival：当前 P/R=`0.500/0.167`，完成线写法为“至少把 revival 事件从 full replace 中分离出来”，即 precision 与 recall 均需高于当前水位。
- provenance_conflict：当前 P/R=`0.000/0.000`，完成线是保守性门禁，按本基准可操作化为“false_positive_count = 0”，也即 provenance 冲突不得再被误判为失效事件。
- non_invalidation_decoy：当前 P/R=`0.000/NA`，完成线是保守性门禁，按本基准可操作化为“false_positive_count = 0”，也即 decoy 子集上不得再咬出任何失效事件。
- out_of_order_late：当前 P/R=`0.000/NA`，完成线写法为“保持 recall 不回退，同时提升 precision”，也即迟到旧观测不再误触发多余失效。
...
- valid-time fallback：当前缺失 observed_at 时回退到达序=`0/0`，完成线是不得回退。

## Retrieval Waterline

- active-set Set-F1：当前水位=`0.848`，先记作基线水位，等待 Phase 1 再看是否与朴素基线拉开分离。
- must_include recall：当前水位=`0.878`，同样先记作基线水位，不在 Phase 0.5 封板时强行拔高。
